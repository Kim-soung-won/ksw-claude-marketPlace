...
4. **에이전트·스킬 `description`은 라벨이 아니라 위임(트리거) 조건이다.** "무엇을 하는가"가 아니라
   "어떤 요청·트리거 문구에 호출되는가"와 예시를 담는다. 에이전트 description(서브에이전트 위임
   판단)과 스킬 description(Claude의 스킬 자동 호출 판단)은 **둘 다 트리거 매칭에 쓰이는 텍스트**이므로
   이 원칙이 동일하게 적용된다. 역할이 겹치는 형제 에이전트·형제 스킬이 있어도 **구분은 자신의
   트리거를 긍정형으로 충분히 구체화해서 해결한다** — 실제로 잘 걸려야 하는 발화·시점·용어를
   촘촘히 적으면 겹침은 자연히 갈린다.
   ...
   > 이름·키워드(형제 에이전트·스킬명, 넘겨야 할 도메인 토큰)가 **오히려 이 에이전트·스킬과 연관**돼
   > 잘못된 호출을 부른다. 부정어는 매칭 전에 대상 토큰을 먼저 심는다. 따라서 description에는
   > 넘길 대상의 이름·토큰을 **아예 쓰지 않는다.** 형제로의 **위임·구분 안내가 꼭 필요하면
   > 본문(에이전트 동작 원칙 / SKILL.md 본문)에 둔다** — 본문은 트리거 매칭에 쓰이지 않으므로 오염이 없다.
