...
| 부정 경계문 | 넘겨야 할 형제 에이전트명·배제 도메인을 "~는 아니다/~와는 다르다"처럼 부정문으로 description에 심지 않았는가 (아이러니 과정 이론 / 분홍 코끼리 효과 — 배제 토큰이 오히려 잘못된 호출을 유발) |
...
- description에 부정 경계문(형제 에이전트명·배제 도메인을 부정문으로 명시)이 있어 잘못된 호출을 유발할 수 있음(분홍 코끼리 효과)
