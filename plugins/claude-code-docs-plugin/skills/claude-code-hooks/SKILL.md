...
| `transcript_path` | **세션 JSONL 경로.** 비동기로 기록되므로 메모리상 대화보다 뒤처질 수 있다 |
| `cwd` | hook이 호출된 시점의 작업 디렉토리 |
| `hook_event_name` | 발생한 이벤트 이름 |
| `permission_mode` | `default` / `plan` / `acceptEdits` / `auto` / `dontAsk` / `bypassPermissions` |
| `prompt_id` | 처리 중인 사용자 프롬프트의 UUID. 첫 입력 전에는 없음 (v2.1.196+) |
| `effort` | 활성 effort 레벨을 담은 `{ level }` 객체. 도구 사용 맥락의 이벤트에 존재 |

서브에이전트 안에서 실행되거나 `--agent` 로 띄운 세션이면 두 필드가 더 붙는다:
`agent_id`(서브에이전트 식별자), `agent_type`(에이전트 이름).

> 세션 JSONL이 필요하면 `transcript_path`를 쓴다. `~/.claude/projects`를 직접 뒤져
> 파일명으로 찾는 방식은 저장 경로 레이아웃에 결합되고 동명 파일을 잘못 집을 수 있다.
